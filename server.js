...
const MEMORY_QUERY_TIMEOUT_MS = Number(process.env.MEMORY_QUERY_TIMEOUT_MS || 3500);
...
function runMemoryCommand(command, payload, timeoutMs = MEMORY_QUERY_TIMEOUT_MS) {
  if (!MEMORY_ENABLED) {
  ...
  for (const executable of pythonCandidates()) {
    ...
    if (result.error && result.error.code === "ETIMEDOUT") {
      return { ok: false, timeout: true, error: "Chroma memory lookup timed out; falling back to web search." };
    }
...
function runMemoryCommandAsync(command, payload) {
  if (!MEMORY_ENABLED) return;

  const script = path.join(ROOT, "scripts", "vector_store.py");
  const input = JSON.stringify(payload);
  const executable = pythonCandidates()[0];
  const child = spawn(executable, [script, command], {
    cwd: ROOT,
    env: memoryEnv(),
...
function storeMemoryAsync(result, query) {
  const record = {
    ...result,
    storedAt: new Date().toISOString()
  };
  runMemoryCommandAsync("upsert", {
    query,
...
async function analyzeFood(payload) {
  ...
  const memory = storeMemoryAsync(result, memoryQuery || hint);
  result.memory = {
    queued: Boolean(memory.queued),
    stored: false,
    error: ""
  };
  ...
  try {
    ...
    if (url.pathname === "/api/health") {
      sendJson(res, 200, {
        ...
        memoryMinScore: MEMORY_MIN_SCORE
      });
      return;
    }
